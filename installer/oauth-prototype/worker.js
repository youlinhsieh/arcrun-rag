  ...
  try {
    ...
    if (bad.length) {
      ...
      throw new InstallError(
        ...
        {
          // 🔴 inkstone/InkStoneCo#141：**名字要在人看得到的地方**。
          //    舊文案只說「有 N 條沒裝成」，名字埋在摺疊的「技術細節」裡
          //    ⇒ 使用者（與回報問題時的我們）看不出是哪一支壞了，也就答不出
          //    「我按更新是為了拿某個修法，它到底有沒有到我身上」。
          //    ⚠️ 只動 `live.routed`（真的壞掉）這一支；另一支是會自己好的狀況，
          //      Arcrun#190 的紅線是兩者不准講成同一句，這裡沒有碰它。
          hint: live.routed
            ? `沒裝成的是：${bad.map((b) => b.name).join('、')}。`
              + '請按「重新安裝」再試一次（已裝好的不會重複）；'
              + '如果同一支一直失敗，把這頁的技術細節回報給我們。'
